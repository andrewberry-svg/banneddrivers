...
def _build_mailgun_assignment_attachment(
    assignments: List[Dict[str, Any]]
) -> Optional[Tuple[str, io.BytesIO]]:
    """Generate an in-memory CSV attachment for the banned assignments."""
    ...
    for assignment in assignments:
        ...
        writer.writerow(
            [
                ...
                vehicle.get("id", "N/A"),
                assignment.get("startTime", "Unknown"),
                assignment.get("endTime", "Unknown"),
            ]
        )

    attachment_bytes = csv_buffer.getvalue().encode("utf-8")
    csv_buffer.close()

    file_buffer = io.BytesIO(attachment_bytes)
    file_buffer.seek(0)

    timestamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    filename = f"banned-driver-assignments-{timestamp}.csv"
    return filename, file_buffer


def _build_mailgun_variables(
    assignments: List[Dict[str, Any]],
    subject: str,
    ...
    for assignment in assignments:
        ...
        summary_lines.append(
            ...
            f"assigned to {vehicle.get('name', 'N/A')} (ID: {vehicle.get('id', 'N/A')}) "
            f"from {start} to {end}"
        )

    content = "\n".join(summary_lines) or "No additional assignment details available."
    variables: Dict[str, Any] = {
        "NotificationTitle": secrets.get("MAILGUN_NOTIFICATION_TITLE")
        or subject,
        "NotificationSubtitle": secrets.get("MAILGUN_NOTIFICATION_SUBTITLE")
        or f"As of {now.strftime('%Y-%m-%d %H:%M:%SZ')} UTC",
        "DevNotificationMainContent": content,
    }

    extra_vars = secrets.get("MAILGUN_TEMPLATE_VARIABLES")
...
def _send_mailgun_notification(assignments: List[Dict[str, Any]], secrets: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from_name = secrets.get("MAILGUN_FROM_NAME", "Fleet Alerts")
    subject = secrets.get("MAILGUN_SUBJECT") or "Banned driver assignments detected"
    template_name = secrets.get("MAILGUN_TEMPLATE_NAME") or secrets.get("MAILGUN_TEMPLATE")
    if not template_name:
        template_name = "sis-email-template"
    ...
    variables = _build_mailgun_variables(assignments, subject, secrets)
    ...
    attachment_buffer: Optional[io.BytesIO] = None
    files = None
    attachment = _build_mailgun_assignment_attachment(assignments)
    if attachment:
        filename, attachment_buffer = attachment
        files = [("attachment", (filename, attachment_buffer))]
