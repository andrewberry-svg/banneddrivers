...
def print_all_drivers():
    """
    Fetch all drivers from the Samsara API and print their information.
    """
    ...
    for driver in drivers:
        ...
        driver_username = driver.get('username', 'N/A')
        print(f"Driver: {driver_name} (ID: {driver_id}, Username: {driver_username})")
    print(f"\nTotal drivers found: {len(drivers)}")

def signout_all_drivers():
    """
    Fetch all drivers from the Samsara API and remotely sign out all of them.
    """
    function = samsara.Function()
    secrets = function.secrets().load()
    api_token = secrets['SAMSARA_API']
    ...
    for driver in drivers:
        ...
        try:
            ...
            print(f"Successfully signed out driver {driver_name} (ID: {driver_id})")
            total_signouts += 1
        except requests.exceptions.RequestException as e:
            ...
            print(f"Unexpected error signing out driver {driver_name} (ID: {driver_id}): {e}")
...
def manage_drivers_and_signout_handler(event, context):
    ...
    import sys
    import json
    # Capture print output for drivers list
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "drivers":
        ...
        print("Usage:")
        print("  python handler.py drivers   # Print all drivers")
