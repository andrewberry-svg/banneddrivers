...
def _fetch_all_drivers(api_token: str, base_url: str) -> List[Dict[str, Any]]:
    """
    Retrieve the complete list of drivers in the organization.
    ...
    Returns:
        List of driver records as returned by the Samsara API.
    """
...
def signout_currently_assigned_drivers():
    """
    Fetch the full driver roster via the list drivers endpoint and remotely sign out each driver.
    """
    function = samsara.Function()
    secrets = function.secrets().load()
    api_token = secrets["SAMSARA_API"]
    ...
    drivers = _fetch_all_drivers(api_token, base_url)
    ...
    for driver in drivers:
        ...
        if not driver_id:
            ...
            print(f"Skipping driver {driver_name} because no ID was provided in the record.")
            continue
        ...
        try:
            ...
            if signout_response.ok:
                print(f"API response: {signout_response.text}")
                total_signouts += 1
            else:
                failed_signouts += 1
                print(
                    f"Failed to sign out driver {driver_name} (ID: {driver_id}). "
                    f"Status: {signout_response.status_code}, Body: {signout_response.text}"
                )
    ...
    if failed_signouts:
        print(f"Total drivers that failed to sign out: {failed_signouts}")
    if skipped_without_id:
        print(f"Drivers skipped due to missing IDs: {skipped_without_id}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "assignments":
        ...
        print("  python handler.py signout               # Sign out all drivers via the list drivers API")
