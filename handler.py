...
def _send_mailgun_notification(assignments: List[Dict[str, Any]], secrets: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from_name = secrets.get("MAILGUN_FROM_NAME", "Samsara Alerts")
    subject = secrets.get("MAILGUN_SUBJECT") or "Banned driver assignments detected"
    template_name = secrets.get("MAILGUN_TEMPLATE_NAME")

    # Build assignment details for email content
    assignment_lines = []
    ...
    main_content = f"The following banned driver assignments were detected:\n\n{assignment_text}"

    # Prepare email data
    email_data = {
        "from": f"{from_name} <{from_email}>",
        "to": ", ".join(recipients),  # Mailgun expects comma-separated string for multiple recipients
        "subject": subject,
    }

    # Use template if provided, otherwise use plain text
    if template_name:
        email_data["template"] = template_name
        # Add template variables
        variables = {
            "DevNotificationMainContent": main_content,
            "NotificationTitle": subject,
            "NotificationSubtitle": f"Found {len(assignments)} banned assignment(s)",
        }
        email_data["h:X-Mailgun-Variables"] = json.dumps(variables)
    else:
        # Fallback to plain text if no template
        email_data["text"] = main_content
    ...
    try:
        ...
        if not response.ok:
            raise Exception(
                f"report email could not be send: status_code={response.status_code} text={response.text}"
            )
