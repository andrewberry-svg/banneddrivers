...
def print_all_drivers():
    """
    Fetch all drivers from the fleet and print their information.
    """
...
def signout_all_drivers():
    """
    Fetch all active drivers from the fleet and remotely sign them out.
    """
    function = samsara.Function()
    secrets = function.secrets().load()
    api_token = secrets['SAMSARA_API']
    ...
    for driver in drivers:
        ...
        # Skip deactivated drivers as they can't be signed in
        if is_deactivated:
            print(f"Skipping driver {driver_name} (ID: {driver_id}) - driver is deactivated")
            total_skipped += 1
            continue
        ...
        try:
            ...
            print(f"  API response status: {signout_response.status_code}")
            if signout_response.status_code in [200, 201, 204]:
    ...
    print(f"Skipped (deactivated or no ID): {total_skipped}")
    print(f"Errors: {total_errors}")
    print("=" * 80)

def manage_drivers_and_signout_handler(event, context):
    """
    Lambda/Function handler that lists all drivers and signs them out.
    """
    import io
    import sys
    import json
    
    # Capture print output for driver list
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "list":
        ...
        print("  python handler.py signout   # Sign out all active drivers")
