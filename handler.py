...
def _send_mailgun_notification(assignments: List[Dict[str, Any]], secrets: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from_name = secrets.get("MAILGUN_FROM_NAME") or "Samsara Alerts"
    ...
    template_name = secrets.get("MAILGUN_TEMPLATE")
    ...
    # Use template if configured, otherwise use plain text
    if template_name:
        data["template"] = template_name
        # Build template variables
        variables = {
            "DevNotificationMainContent": body_text,
            "NotificationSubtitle": f"Detected {len(assignments)} banned driver assignment(s)",
            "NotificationTitle": "Banned Driver Alert",
        }
    ...
    try:
        ...
        if not response.ok:
            reason = f"Mailgun API error: {response.status_code=} {response.text=}"
            print(f"Failed to send Mailgun notification: {reason}")
            status["reason"] = reason
            return status
